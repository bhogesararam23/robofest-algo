...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument('start_gazebo', default_value='true'), DeclareLaunchArgument('start_sitl', default_value='false'),
        ExecuteProcess(cmd=['gz','sim','-r',world], output='screen', condition=IfCondition(LaunchConfiguration('start_gazebo'))),
        ExecuteProcess(cmd=['ardupilot','--model','quad'], output='screen', condition=IfCondition(LaunchConfiguration('start_sitl'))),
